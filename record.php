        ...
        throw new Exception('appkey incorrect');
            ...
            if (empty($keyValue)){
                ...
                if (empty($result)){
                    ...
                    //同时将查询结果序列化后存入redis，键名为urlencode后的$name
                    $data = json_encode($result,JSON_UNESCAPED_UNICODE);
                    $resultTOString = serialize($result);
                    $redis->set($keyName,$resultTOString,86400);
                }

            } else {
                $resultTOString =  $redis->get($keyName);
                $resultTOArray = unserialize($resultTOString);
                $data = json_encode($resultTOArray,JSON_UNESCAPED_UNICODE);
            }
        }
